  ...
  <div class="content-container">
        ...
        <table class="table">
          ...
          <tbody>
            <tr v-for="(scheme, index) in pagedSchemes" :key="scheme.id"
                :class="{
                  'bg-green-50': scheme.status,
                  'bg-gray-50': scheme.type === 'province' && !scheme.status
                }"
                :style="scheme.type === 'platform' && scheme.status ? {backgroundColor: '#f0fdf4'} : scheme.type === 'province' && !scheme.status ? {backgroundColor: '#f9fafb'} : {}">
              ...
              <td>
                <div style="display: flex; align-items: center; gap: 8px;">
                  ...
                  <span v-if="scheme.status" style="padding: 2px 8px; background: #dcfce7; color: #15803d; border-radius: 4px; font-size: 11px;">启用中</span>
                </div>
              </td>
              <td>
              ...
              <td>
                <label class="switch">
                  <input type="checkbox" :checked="scheme.status" :disabled="isLastEnabledPlatform(scheme)" @change="toggleStatus(scheme.id)">
    ...
    <div v-if="showAddModal" class="modal-overlay" @click.self="closeAddModal" style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); display: flex; justify-content: center; align-items: center; z-index: 1000;">
      <div style="background: white; border-radius: 12px; width: 450px; padding: 24px;">
        ...
        <div class="filter-item mb-3">
          <label>类型 *</label>
          <select class="select" v-model="newSchemeType">
            <option value="">请选择类型</option>
            <option value="platform">平台</option>
            <option value="province">省级</option>
          </select>
        </div>
        <div v-if="newSchemeType === 'province'" class="filter-item mb-3">
          <label>省份 *</label>
          <select class="select" v-model="newSchemeProvince">
            <option value="">请选择省份</option>
            <option v-for="prov in provinceList" :key="prov" :value="prov">{{ prov }}</option>
          </select>
        </div>
        <div v-if="newSchemeType === 'institution'" class="filter-item mb-3">
          <label>机构 *</label>
          <select class="select" v-model="newSchemeInstitution">
            <option value="">请选择机构</option>
            <option v-for="inst in institutionList" :key="inst" :value="inst">{{ inst }}</option>
          </select>
        </div>
        <div class="filter-item mb-3">
          <label>培训阶段 *</label>
    ...
    <div v-if="showProjectModal && currentStationContext" class="modal-overlay" @click.self="showProjectModal = false" style="position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.5);display:flex;justify-content:center;align-items:center;z-index:1000;">
      <div class="modal-content" style="background:white;border-radius:12px;width:400px;padding:20px;">
        ...
        <div style="border:1px solid var(--border);border-radius:8px;padding:10px;max-height:300px;overflow-y:auto;">
          <div v-for="item in availableForProject" :key="item.name" style="margin-bottom:8px;">
            <label style="display:flex;align-items:center;gap:8px;cursor:pointer;">
              <input type="checkbox" :value="item.name" v-model="projectSelected" style="width:16px;height:16px;">
              <span>{{ item.name }}</span>
            </label>
          </div>
...
onMounted(async () => {
  try {
    ...
    console.log('✅ 考站方案加载成功:', data.length, '个方案')
    schemes.value = data
  } catch (e) {
...
const newSchemeType = ref('')
const newSchemeProvince = ref('')
...
const institutionList = ['清华长庚医院', '浙江省人民医院', '仁爱医院', '华西医院', '中山医院', '协和医院', '同济医院', '省立医院', '瑞金医院', '北大第一医院', '湘雅医院', '齐鲁医院', '郑大一附院', '南方医院', '西南医院']
...
const commonStationNames = ['接诊病人站', '病历书写站', '病例分析站', '技能操作站', '体格检查站', '急救技能站', '辅助检查站', '临床思维站', '交流沟通站', '病史采集站', '接诊和沟通站']
...
// 从评分表管理中获取的可用评分表列表（mock 数据）
const scoreTableAvailable = [
  { template_code: 'TPL-001', template_name: '内科问诊评分表', specialty: '内科', usage_count: 156 },
  { template_code: 'TPL-005', template_name: '外科体格检查评分表', specialty: '外科', usage_count: 89 },
  { template_code: 'TPL-012', template_name: '儿科急诊评分表', specialty: '儿科', usage_count: 0 },
  { template_code: 'TPL-008', template_name: '妇产科常规评分表', specialty: '妇产科', usage_count: 234 },
  { template_code: 'TPL-023', template_name: '全科综合能力评分表', specialty: '全科', usage_count: 12 },
  { template_code: 'TPL-019', template_name: '急诊科抢救流程评分表', specialty: '急诊科', usage_count: 67 },
  { template_code: 'TPL-STD', template_name: '病史采集评分表（标准模板）', specialty: '通用', usage_count: 428 },
  { template_code: 'TPL-STD-2', template_name: '内科-体格检查评分表', specialty: '内科', usage_count: 312 },
  { template_code: 'TPL-STD-3', template_name: '病历书写评分表', specialty: '通用', usage_count: 198 },
  { template_code: 'TPL-STD-4', template_name: '内科-人文沟通评分表', specialty: '内科', usage_count: 145 },
  { template_code: 'TPL-STD-5', template_name: '外科-初步诊断评分表', specialty: '外科', usage_count: 88 },
  { template_code: 'TPL-STD-6', template_name: '外科-人文沟通评分表', specialty: '外科', usage_count: 73 }
]

const filteredScoreTableOptions = computed(() => {
  const kw = scoreTableSearchKeyword.value.trim().toLowerCase()
  if (!kw) return scoreTableAvailable
  return scoreTableAvailable.filter(st =>
...
const projectCandidates = [
  { name: '病史采集', defaultScore: 30 },
  { name: '体格检查', defaultScore: 30 },
  { name: '人文沟通', defaultScore: 20 },
  { name: '初步诊断', defaultScore: 20 },
  { name: '书写病历', defaultScore: 50 },
  { name: '病例分析', defaultScore: 100 }
]

const filteredSchemes = computed(() => {
  let list = schemes.value.filter(s => s.type !== 'institution')
...
const availableForProject = computed(() => {
  ...
  return projectCandidates.filter(c => !existNames.has(c.name))
})
...
function isLastEnabledPlatform(scheme) {
  if (scheme.type !== 'platform' || !scheme.status) return false
  const platformEnabled = schemes.value.filter(s => s.type === 'platform' && s.status)
  return platformEnabled.length <= 1
}

async function toggleStatus(schemeId) {
  const target = schemes.value.find(s => s.id === schemeId)
  if (!target) return
  if (target.status) {
    // 禁用前检查：平台方案必须保留至少一个启用
    if (target.type === 'platform') {
      const platformEnabled = schemes.value.filter(s => s.type === 'platform' && s.status)
      if (platformEnabled.length <= 1) {
        toast.show('必须保留至少一个启用的平台方案，无法禁用。', 'warning')
        return
      }
    }
    target.status = false
    await saveSchemeEdit(target)
  } else {
    confirm(`确定要启用"${target.name}"吗？启用后将自动禁用同类型其他方案。`).then(async ok => {
      if (ok) {
        if (target.type === 'platform') {
          for (const s of schemes.value.filter(s => s.type === 'platform')) { s.status = false; await saveSchemeEdit(s) }
        } else if (target.type === 'province') {
          for (const s of schemes.value.filter(s => s.type === 'province' && s.source === target.source)) { s.status = false; await saveSchemeEdit(s) }
        }
        target.status = true
        await saveSchemeEdit(target)
      }
...
async function copyScheme(schemeId) {
  ...
  const prefix = original.type === 'platform' ? 'res-platform-' : original.type === 'province' ? 'res-province-' : 'res-inst-'
  copy.id = prefix + Date.now()
...
function openAddModal() {
  ...
  newSchemeName.value = ''
  newSchemeType.value = ''
  newSchemeProvince.value = ''
  newSchemeInstitution.value = ''
}
...
async function saveNewScheme() {
  ...
  if (!newSchemeType.value) { toast.show('请选择方案类型', 'warning'); return }
  if (newSchemeType.value === 'province' && !newSchemeProvince.value) { toast.show('请选择省份', 'warning'); return }
  if (newSchemeType.value === 'institution' && !newSchemeInstitution.value) { toast.show('请选择机构', 'warning'); return }
  const prefix = newSchemeType.value === 'platform' ? 'res-platform-' : newSchemeType.value === 'province' ? 'res-province-' : 'res-inst-'
  const source = newSchemeType.value === 'platform' ? '——' : newSchemeType.value === 'province' ? newSchemeProvince.value : newSchemeInstitution.value
  const scheme = { id: prefix + Date.now(), name: newSchemeName.value, type: newSchemeType.value, source, phase: activePhaseTab.value, status: false, majors: [] }
...
function deleteScheme(schemeId) {
  const scheme = schemes.value.find(s => s.id === schemeId)
  if (scheme.type === 'platform') { toast.show('平台级方案不可删除', 'warning'); return }
...
function deleteStation(majorIndex, stationIndex) {
  if (!editingScheme.value) return
  const stations = editingScheme.value.majors[majorIndex].stations
  if (stations.length <= 1) {
    toast.show('同类考站只剩一个时不能删除', 'warning')
    return
  }
  confirm('确定删除该考站吗？').then(ok => {
    if (ok) stations.splice(stationIndex, 1)
  }).catch(() => {})
}
...
function submitProject() {
  ...
  const { majorIndex, stationIndex } = currentStationContext.value
  const station = editingScheme.value.majors[majorIndex].stations[stationIndex]
  const result = projectSelected.value.map(name => {
    const cand = projectCandidates.find(c => c.name === name)
    return { name, score: cand.defaultScore }
  })
...
function submitScoreTable() {
  ...
  const selected = scoreTableAvailable.find(st => st.template_code === scoreTableSelectedCode.value)
