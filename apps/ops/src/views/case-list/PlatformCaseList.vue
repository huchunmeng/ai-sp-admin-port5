        ...
        <table class="table">
          ...
          <tbody>
            <tr v-for="item in paginatedData" :key="item.id">
              ...
              <td class="sticky-right" style="right:0"><div class="flex gap-2"><button class="btn btn-sm" @click="editCase(item)">编辑</button><button class="btn btn-sm" @click="publishCase(item)">发布</button></div></td>
            </tr>
...
const filters = reactive({
  ...
  ai_quality_status: '', editor_review_status: '', expert_review_status: '', base_review_status: '',
  source: '', institution: ''
})
...
const availableCategories = computed(() => filters.specialty ? dict.categoriesMap[filters.specialty] || [] : [])
const availableDiseases = computed(() => filters.category ? dict.diseasesMap[filters.category] || [] : [])
...
const mockData = () => [
  { id:1, case_id:'ORT-20260417-LDH03', title:'腰椎间盘突出症（L3/4节段）', teaching_phase:'住院医师', specialty:'骨科/康复科', category:'脊柱外科', disease:'腰椎间盘突出症', difficulty:'L3', version:'v2.1.0', ai_quality_status:'已通过', editor_review_status:'已通过', expert_review_status:'已通过', base_review_status:'已通过', source:'平台', institution:'仁爱医院', creator_name:'张医生', created_at:'2026-03-15 10:30', enabled:true, patient_name:'赵建国', patient_gender:'男', patient_age:'48' },
  { id:2, case_id:'IM-20240520-A1B2', title:'肾病综合症', teaching_phase:'本科教学', specialty:'内科', category:'心内科', disease:'心衰', difficulty:'L3', version:'v1.5.2', ai_quality_status:'已通过', editor_review_status:'待审核', expert_review_status:'待审核', base_review_status:'待审核', source:'机构', institution:'华西医院', creator_name:'李主任', created_at:'2026-04-01 14:20', enabled:true, patient_name:'李建国', patient_gender:'男', patient_age:'62' },
  { id:3, case_id:'IM-20240520-GAS03', title:'胃溃疡伴出血', teaching_phase:'专科培训', specialty:'内科', category:'消化内科', disease:'胃溃疡', difficulty:'L2', version:'v3.0.0', ai_quality_status:'未通过', editor_review_status:'未通过', expert_review_status:'待审核', base_review_status:'待审核', source:'专家', institution:'中山医院', creator_name:'王教授', created_at:'2026-02-28 09:15', enabled:false, patient_name:'张永生', patient_gender:'男', patient_age:'52' },
  { id:4, case_id:'OB-20260120-ND01', title:'正常分娩', teaching_phase:'本科教学', specialty:'妇产科', category:'产科', disease:'正常分娩', difficulty:'L1', version:'v1.2.0', ai_quality_status:'已通过', editor_review_status:'已通过', expert_review_status:'已通过', base_review_status:'已通过', source:'平台', institution:'协和医院', creator_name:'赵医师', created_at:'2025-12-10 16:40', enabled:true, patient_name:'李丽', patient_gender:'女', patient_age:'28' },
  { id:5, case_id:'ORT-20260418-FX05', title:'胫骨平台骨折', teaching_phase:'住院医师', specialty:'外科', category:'骨科', disease:'骨折', difficulty:'L2', version:'v1.0.0', ai_quality_status:'待质检', editor_review_status:'待审核', expert_review_status:'待审核', base_review_status:'待审核', source:'个人', institution:'同济医院', creator_name:'刘医生', created_at:'2026-04-18 11:05', enabled:true, patient_name:'王强', patient_gender:'男', patient_age:'35' },
  { id:6, case_id:'PED-20260301-JX01', title:'儿童哮喘急性发作', teaching_phase:'专科培训', specialty:'儿科', category:'小儿内科', disease:'哮喘', difficulty:'L2', version:'v1.8.0', ai_quality_status:'已通过', editor_review_status:'已通过', expert_review_status:'已通过', base_review_status:'已通过', source:'平台', institution:'省立医院', creator_name:'陈主任', created_at:'2026-03-01 08:30', enabled:true, patient_name:'小明', patient_gender:'男', patient_age:'7' },
]
...
const allColumns = [
  ...
  { key: 'ai_quality_status', label: 'AI质检' }, { key: 'editor_review_status', label: '编辑审核' },
  { key: 'expert_review_status', label: '专家审核' }, { key: 'base_review_status', label: '基地审核' },
  { key: 'source', label: '来源' }, { key: 'institution', label: '来源机构' },
...
const publishCase = (item) => { confirm(`确认发布病例「${item.title}」吗？`).then(() => { item.editor_review_status = '已通过'; toast.show(`病例「${item.title}」已发布`) }).catch(() => {}) }
