<template>
  <div class="content-container exam-create-container">
    ...
    <div v-if="caseSelectorVisible" class="modal-overlay" @click.self="caseSelectorVisible=false" data-review-exempt>
      <div class="modal-container"><h3 class="mb-4" data-reviewable="选择病例标题">选择病例</h3><input v-model="caseKeyword" placeholder="搜索病例名称/编码" class="input-field w-full mb-2"><table class="table"><thead><tr><th>编码</th><th>标题</th><th>专业</th><th>病种</th><th>难度</th><th></th></tr></thead><tbody><tr v-for="c in filteredCases" :key="c.id"><td>{{ c.code }}</td><td>{{ c.title }}</td><td>{{ c.specialty }}</td><td>{{ c.disease }}</td><td>{{ c.difficulty }}</td><td><button @click="selectCase(c)" class="btn-primary btn-sm">选择</button></td></tr></tbody></table></div>
    </div>
            ...
            <div style="border:1px solid var(--border);border-radius:8px;padding:10px;max-height:200px;overflow-y:auto">
              <div v-for="item in scoreSettingsTargetSession?.items_duration || []" :key="item.id" style="margin-bottom:6px">
                <label class="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" :checked="scoreSettingsTargetSt?.items?.includes(item.name)" style="width:16px;height:16px"> {{ item.name }}
                </label>
              </div>
            </div>
          </div>
...
async function loadCases() {
  ...
  try {
    const res = await fetch('/data/cases/cases-index.json')
    if (res.ok) {
      const data = await res.json()
      mockCases.value = data.map(c => ({
        id: c.id || c.case_id,
        code: c.case_id,
        title: c.title,
        specialty: c.specialty,
        disease: c.disease,
        difficulty: c.difficulty || 'L2',
        communicationScenes: []
      }))
...
const filteredCases = computed(() => {
  ...
  return mockCases.value.filter(c => c.title.includes(kw) || c.code.includes(kw))
})
...
const deleteSession = (session) => {
  const sameTypeCount = allSessions.value.filter(s => s.stationName === session.stationName).length
  if (sameTypeCount <= 1) {
...
const selectCase = (c) => {
  if (tempSession) {
    ...
    const isComm = isCommunicationStation(tempSession) || tempSession.stationName.includes('接诊')
      ...
      confirm(`已将病例「${c.title}」分配给「${tempSession.name}」。\n是否同时应用到其他 ${sameTypeStations.length} 个同类型考站？`).then(ok => {
        if (ok) {
          for (const s of sameTypeStations) {
            s.case = c
            const scenes = c.communicationScenes || []
            const isComm = isCommunicationStation(s) || s.stationName.includes('接诊')
...
onMounted(async () => {
  ...
  schemeOptions.value = allSchemes.map(transformScheme)
  schemesLoading.value = false
  if (schemeOptions.value.length > 0) {
    form.value.schemeId = schemeOptions.value[0].id
    form.value.selectedMajors = [...availableMajors.value]
    onSchemeChange()
  }
