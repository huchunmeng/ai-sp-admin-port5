        ...
        <template v-if="scoreSettingTab === 'pass'">
          ...
          <div class="form-item">
            ...
            <div v-if="minFailedEnabled" style="margin-left:24px">
              ...
              <div class="mb-2" v-if="condition2Mode === 'fixed'"><label><input type="radio" v-model="condition2Mode" value="count"> ○至少有 <select class="select" v-model="minPassedStations" style="width:auto;margin:0 4px"><option :value="0">不限</option><option v-for="n in scoreActiveStations.length" :key="n" :value="n">{{ n }}</option></select> 个考站达到合格线</label></div>
              <div class="mb-2" v-if="condition2Mode === 'count'"><label><input type="radio" v-model="condition2Mode" value="fixed"> ○以下考站必须达到合格线：</label></div>
            </div>
          </div>
        </template>
      </div>
...
const examId = route.params.examId || 'exam_1'
const exam = ref({
  id: examId,
  name: '2026年住培结业考核',
  candidate_count: 20
