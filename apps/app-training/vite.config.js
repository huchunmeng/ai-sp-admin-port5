...
export default defineConfig(({ mode }) => {
  ...
  if (LLM_API_KEY && LLM_API_KEY !== 'your-api-key-here') {
    ...
    console.log('[vite] ║                                              ║')
    console.log('[vite] ║  在 apps/app-training/.env.local 中配置：      ║')
  ...
  return {
    ...
    resolve: {
      alias: {
        // app-training 特有文件 (必须在更通用的别名之前)
        '@/stores/appTraining': fileURLToPath(new URL('./src/stores/appTraining', import.meta.url)),
        // 指向训练端 (必须先于 @ 匹配)
        '@/composables': fileURLToPath(new URL('../training/src/composables', import.meta.url)),
        '@/components': fileURLToPath(new URL('../training/src/components', import.meta.url)),
        '@/stores': fileURLToPath(new URL('../training/src/stores', import.meta.url)),
        '@/styles': fileURLToPath(new URL('../training/src/styles', import.meta.url)),
        '@/t-views': fileURLToPath(new URL('../training/src/views', import.meta.url)),
