<template>
  <div class="search-select" ref="componentRef">
    <div class="search-select-input" @click="toggleDropdown"
      ...
      <span style="overflow:hidden;text-overflow:ellipsis">{{ displayText || placeholder || '全部' }}</span>
...
const displayText = computed(() => {
  ...
  const found = opts.find(o => {
    if (typeof o === 'string') return o === props.modelValue
    return o.value === props.modelValue || o.label === props.modelValue
  })
  if (found) return typeof found === 'string' ? found : (found.label || found.value || String(found))
...
function selectOption(opt) {
  const val = (typeof opt === 'object' && opt !== null) ? (opt.value !== undefined ? opt.value : opt) : opt
