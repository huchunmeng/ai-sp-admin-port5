<template>
  <div class="content-container">
    ...
    <div class="mb-4">
      <div class="flex gap-2">
        <button class="btn-primary flex items-center gap-1" @click="createExam">
        ...
        <table class="table">
          ...
          <tbody>
            ...
            <tr v-if="tableData.length === 0">
              <td :colspan="13" class="text-center py-8 text-secondary">暂无数据</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <div v-if="deleteVisible" class="modal-overlay" @click.self="deleteVisible = false">
      <div class="modal-container" style="width: 400px;">
        ...
        <div class="drawer-footer" style="justify-content: flex-end;"><button class="btn" @click="deleteVisible = false">取消</button><button class="btn-danger" @click="confirmDelete">确认删除</button></div>
      </div>
    </div>
...
const filters = ref({ keyword: '', status: '', typeCategory: '', dateFrom: '' })
...
const handleReset = () => { filters.value = { keyword: '', status: '', typeCategory: '', dateFrom: '' }; handleSearch() }
const createExam = () => { router.push({ name: 'examCreate' }) }
const editExam = (id) => { router.push({ name: 'examCreate', query: { examId: id } }) }
...
const openMonitor = (exam) => {
  router.push({ name: 'examMonitor', params: { examId: exam.id } })
}
</script>
