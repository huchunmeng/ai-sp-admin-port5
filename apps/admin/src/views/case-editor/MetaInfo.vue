  ...
  <div style="padding:10px;">
    <div v-if="meta">
      <div class="card" style="padding:20px; margin-bottom:20px;">
        <h3 style="margin-bottom:12px;">🧠 患者性格特征</h3>
        <div class="info-grid">
          <div class="info-item">
            <label>情绪外显程度</label>
            <select class="select" :value="personality.expressiveness" @change="emitPersonality('expressiveness', $event.target.value)">
              <option>火爆型</option><option>普通型</option><option>偏内敛</option><option>隐忍型</option>
            </select>
          </div>
          <div class="info-item">
            <label>敏感度</label>
            <select class="select" :value="personality.sensitivity" @change="emitPersonality('sensitivity', $event.target.value)">
              <option>高敏感</option><option>普通敏感度</option><option>钝感</option>
            </select>
          </div>
          <div class="info-item">
            <label>恢复力</label>
            <select class="select" :value="personality.resilience" @change="emitPersonality('resilience', $event.target.value)">
              <option>高豁达</option><option>普通恢复力</option><option>低豁达</option>
            </select>
          </div>
        </div>
      ...
      <div class="card" style="padding:20px; margin-bottom:20px;">
        ...
        <div class="info-grid">
          ...
          <div class="info-item"><label>难度</label><span>{{ safeMeta.difficulty ? getDifficultyLabel(safeMeta.difficulty) + ' · ' + getCaseLevelLabel(safeMeta.difficulty) : '—' }}</span></div>
...
const personality = computed(() => {
  const p = props.meta?.personality || {}
  return {
    expressiveness: p.expressiveness || '普通型',
    sensitivity: p.sensitivity || '普通敏感度',
    resilience: p.resilience || '普通恢复力'
  }
})
...
const PERSONALITY_DESC = {
  '火爆型': '情绪外露，一点就着，但也消得快',
  '普通型': '情绪表达适中',
  '偏内敛': '外表相对平静，内心有情绪也不直接表达',
  '隐忍型': '习惯克制，不轻易表露情绪，但一旦爆发收不住',
  '高敏感': '容易因学生言行感到受伤，细微的冷漠或敷衍都能察觉并放大',
  '普通敏感度': '对学生的言行有正常的情绪反应',
  '钝感': '不太容易被学生的言行影响',
  '高豁达': '负面情绪消退较快，不记仇，学生道歉后容易原谅',
  '普通恢复力': '情绪恢复速度正常',
  '低豁达': '负面情绪消退较慢，记仇，即使道歉也需要更长时间释怀'
}

const personalityDesc = computed(() => {
  const expr = personality.value.expressiveness
  const sens = personality.value.sensitivity
  const res = personality.value.resilience
  const parts = []
  if (expr !== '普通型') parts.push(PERSONALITY_DESC[expr])
  if (sens !== '普通敏感度') parts.push(PERSONALITY_DESC[sens])
  if (res !== '普通恢复力') parts.push(PERSONALITY_DESC[res])
  return parts.length ? parts.join('；') : '默认性格：情绪表达、敏感度、恢复力均为普通水平。'
})

function emitPersonality(dim, value) {
  emit('update:meta', {
    ...props.meta,
    personality: { ...personality.value, [dim]: value }
  })
}
