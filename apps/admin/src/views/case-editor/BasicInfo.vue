<template>
  <div>
    ...
    <div class="filter-row mb-4">
      ...
      <div class="filter-item" style="min-width:100px"><label>性别</label><select class="select" :value="formData.patient_gender" @change="onGenderChange($event.target.value)"><option>男</option><option>女</option></select></div>
      ...
      <div class="filter-item" style="min-width:100px"><label>教育程度</label><select class="select" :value="formData.patient_education" @change="updateField('patient_education', $event.target.value)"><option value="">未知</option><option value="小学">小学</option><option value="初中">初中</option><option value="高中">高中</option><option value="大专">大专</option><option value="本科">本科</option><option value="研究生">研究生</option></select></div>
      ...
      <div class="filter-item" style="min-width:140px" v-if="formData.patient_gender === '女'"><label>孕期</label><select class="select" :value="formData.patient_pregnancy" @change="updateField('patient_pregnancy', $event.target.value)"><option>未怀孕</option><option>孕早期</option><option>孕中期</option><option>孕晚期</option></select></div>
      ...
      <div class="filter-item" style="min-width:120px"><label>沟通对象</label><select class="select" :value="formData.communication_target" @change="updateField('communication_target', $event.target.value)"><option value="patient">患者本人</option><option value="family">家属</option></select></div>
    </div>
    <div class="filter-row mb-4">
    ...
    <div class="filter-item mb-4"><label>专科重点</label><textarea class="input" style="width:100%" :value="formData.specialty_focus" @input="updateField('specialty_focus', $event.target.value)" placeholder="本专科在该病例中需要重点关注的诊疗要点..."></textarea></div>
  </div>
</template>
...
function onGenderChange(val) {
  const updates = { patient_gender: val }
  if (val === '男') updates.patient_pregnancy = '未怀孕'
