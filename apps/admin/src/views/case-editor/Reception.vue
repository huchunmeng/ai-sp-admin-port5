        ...
        <div v-show="activeSection === 'examiner'">
          ...
          <div class="content-body">
            ...
            <div class="score-section">
              <div class="score-section-header">
                <h4>诊断与鉴别</h4>
                <span style="font-size:11px;color:var(--text-tertiary)">诊断数据从基础信息同步</span>
              </div>
              <table class="score-table">
                <thead><tr><th>类型</th><th>诊断名称</th><th style="width:100px">分值</th></tr></thead>
                <tbody>
                  <tr v-for="(d, idx) in diagnosisTable" :key="idx">
                    <td>
                      <span :class="d.type === 'primary' ? 'badge badge-info' : 'badge badge-warning'" style="font-size:11px">
                        {{ d.type === 'primary' ? '初步诊断' : '鉴别诊断' }}
                      </span>
                    </td>
                    <td style="color:var(--text-main);font-size:13px">{{ d.name }}</td>
                    <td>
                      <input v-if="d.type === 'primary'" class="input" type="number" min="0" step="0.5" v-model.number="formData.reception.examiner_materials.diagnosis_answer.primary_score" style="width:80px">
                      <input v-else class="input" type="number" min="0" step="0.5" v-model.number="formData.reception.examiner_materials.diagnosis_answer.differential_score" style="width:80px">
                    </td>
                  </tr>
                  <tr v-if="diagnosisTable.length === 0">
                    <td colspan="3" class="score-empty">暂无诊断数据，请在基础信息中添加</td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            <div class="score-section">
              <div class="score-section-header">
                <h4>阳性体征</h4>
                <button class="btn btn-sm" @click="addPositiveSign">+ 添加</button>
              </div>
              <div style="padding:12px">
                <div v-if="positiveSignEntries.length === 0" style="text-align:center;padding:16px;color:var(--text-tertiary);font-size:13px;">暂无阳性体征数据</div>
                <table v-else class="score-table">
                  <thead><tr><th>#</th><th>检查项目</th><th>结果描述</th><th style="width:80px">分值</th><th style="width:50px"></th></tr></thead>
                  <tbody>
                    <tr v-for="(entry, idx) in positiveSignEntries" :key="idx">
                      <td>{{ idx + 1 }}</td>
                      <td><input class="input" v-model="entry.key" :placeholder="'检查项目'" style="width:100%"></td>
                      <td><input class="input" v-model="entry.value" :placeholder="'结果描述...'" style="width:100%"></td>
                      <td><input class="input" type="number" min="0" step="0.5" v-model.number="entry.score" style="width:80px"></td>
                      <td><button class="btn-icon-sm" @click="deletePositiveSign(idx)">🗑️</button></td>
                    </tr>
                  </tbody>
                </table>
              </div>
...
function cloneQATurns(turns) {
  return turns.map(t => {
    // 归一化：doctor/examiner/question → doctor, patient/answer → patient
    const dr = t.doctor ?? t.examiner ?? t.question
    const pt = t.patient ?? t.answer
    if (dr !== undefined && pt !== undefined) {
      return { doctor: dr || '', patient: pt || '' }
    }
    if (t.patient_active !== undefined) {
      return { patient_active: t.patient_active || '', note: t.note || '' }
    }
    if (dr !== undefined) {
      return { doctor: dr || '', patient: '' }
    }
    if (pt !== undefined) {
      return { doctor: '', patient: pt || '' }
    }
    return {}
  })
}
...
const spRoleInfo = computed({
  ...
  set(val) {
    if (!props.formData.reception.sp_materials) {
      props.formData.reception.sp_materials = { role_info: val, self_narration: '', qa_script: [], role: spRole.value }
    }
...
const diagnosisTable = computed(() => {
  const rows = []
  const primary = props.formData.primary_diagnosis || []
  primary.forEach(p => rows.push({ type: 'primary', name: p }))
  const diff = props.formData.differential_diagnosis || []
  diff.forEach(d => rows.push({ type: 'differential', name: d }))
  return rows
})

const positiveSignEntries = ref([])

function syncPositiveSignsToForm() {
  internalUpdate.value = true
  const signs = {}
  positiveSignEntries.value.forEach(e => { signs[e.key] = { value: e.value, score: e.score || 0 } })
  props.formData.reception.examiner_materials.positive_signs = signs
  nextTick(() => { internalUpdate.value = false })
}

watch(positiveSignEntries, () => { syncPositiveSignsToForm() }, { deep: true })

watch(
  () => props.formData.reception.examiner_materials.positive_signs,
  (signs) => {
    if (!internalUpdate.value) {
      positiveSignEntries.value = Object.entries(signs || {}).map(([key, val]) => ({
        key,
        value: typeof val === 'object' ? (val.value || '') : (typeof val === 'string' ? val : ''),
...
function addPositiveSign() {
  positiveSignEntries.value.push({ key: '', value: '', score: 0 })
}

function deletePositiveSign(idx) {
  positiveSignEntries.value.splice(idx, 1)
}

let syncTimer = null
function debouncedSyncToForm() {
