        ...
        <div v-if="currentScene" class="content-body">
          <div class="scene-basic-row">
            ...
            <div class="scene-field scene-field-name">
              <label>场景名称（10字以内）</label>
              <input class="input" v-model="currentScene.scenario_name" placeholder="如：告知病情、方案沟通" maxlength="10">
            </div>
          ...
          <div v-if="activeSubKey === 'sp'">
            <h4 class="section-title">标准化病人材料</h4>
            ...
            <div class="script-section">
              ...
              <div class="script-list">
                <div v-for="(turn, idx) in currentScene.sp_materials.script" :key="idx" class="script-card">
                  ...
                  <div class="mb-2">
                    <label class="field-mini-label">对话内容</label>
                    <textarea class="input" v-model="turn.line" placeholder="请输入对话内容..."></textarea>
                  </div>
                  <div class="script-card-fields">
                    <div class="flex-1">
                      <label class="field-mini-label">情绪</label>
                      <input class="input" v-model="turn.emotion" placeholder="如：焦虑、轻松">
                    </div>
                  </div>
                  <div class="script-card-note">
                    <label class="field-mini-label">备注</label>
                    <textarea class="input" v-model="turn.note" placeholder="可选备注"></textarea>
                  </div>
                </div>
          ...
          <div v-else-if="activeSubKey === 'examiner'">
            ...
            <div class="dialogue-section">
              <div class="script-section-header">
                <label class="section-label">考官完整对话（标准化病人与考生问答组）</label>
              ...
              <div class="dialogue-groups">
                <div v-for="(pair, idx) in qaPairsList" :key="idx" class="dialogue-group">
                  ...
                  <div class="filter-item mb-2">
                    <label class="field-mini-label">标准化病人提问</label>
                    <textarea class="input" v-model="pair.sp_line" @input="syncQAPairsImmediate" placeholder="标准化病人发言内容..."></textarea>
                  </div>
                  <div class="filter-item mb-2">
                    <label class="field-mini-label">标准化病人情绪</label>
                    <input class="input" v-model="pair.sp_emotion" @input="syncQAPairsImmediate" placeholder="如：痛苦、焦虑">
                  </div>
                  <div class="filter-item mb-2">
                    <label class="field-mini-label">考生预期回应（简述）</label>
...
function addScenario() {
  const maxNum = props.formData.humanity.scenarios.reduce((m, s) => {
    const match = (s.scenario_id || '').match(/S(\d+)/)
    return match ? Math.max(m, parseInt(match[1])) : m
  }, 0)
  const scene = createEmptyScenario()
  scene.scenario_id = 'S' + String(maxNum + 1).padStart(3, '0')
  props.formData.humanity.scenarios.push(scene)
