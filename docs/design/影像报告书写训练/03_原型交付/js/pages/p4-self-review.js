...
(function (w) {
  ...
  function P4SelfReview() {
    ...
    return '<div class="content-inner">' +
      ...
      /* ── T0 阅片笔记回看（§5.2.2「阅片笔记（T0）定位」：不进入报告、不参与评分，T4 可回看） ── */
      '<div class="card">' +
        ...
        '<div class="card-body">' +
          (S.WORKBENCH.viewNotes
            ? '<div style="background:var(--border-light);border-radius:10px;padding:11px 15px;' +
                'font-size:13px;line-height:1.8;color:var(--text-main)">' +
                H.esc(S.WORKBENCH.viewNotes) + '</div>'
            : '<div class="tiny muted">本回合未记阅片笔记（T0 为可选填）。</div>') +
          '<div class="tiny muted mt8">' + H.rich('回看用途：对照"**当时看到了什么**"与"**报告里写没写**"，帮助定位遗漏的征象。') + '</div>' +
        '</div>' +
      '</div>' +
