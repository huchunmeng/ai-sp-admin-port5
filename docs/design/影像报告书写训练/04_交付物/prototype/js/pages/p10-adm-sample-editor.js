/* P10 样本编辑器（管理端）· PRD §5.12.3–5.12.6 / §6.1 路由 /imaging-samples/:id
   一页把「入库 → 脱敏 → 能力位 → 金标准 → 发布」串完；改已发布样本强制新建版本（§5.12.8）。 */
(function (w) {
  'use strict';
  var S = w.SEED, H = w.H;
  var U = w.ADMUI;
  ...
  function P10AdmSampleEditor() {
    ...
    return '<div class="content-inner">' +
      ...
      (isVerBump ? '<div class="banner warn">' + H.icon('warn', { size: 15 }) +
        '<span>' + H.rich('本样本**已发布**（v' + s.version + '）。改动**不允许原地覆盖**，保存即**新建版本 v' + (s.version + 1) + '**（§5.12.8）；' +
          '已有 **' + imp.openTasks + ' 个任务**锁在 v' + s.version + '，学生看到的影像与评分依据不变。') + '</span></div>' : '') +

      /* ── §5.12.3 影像序列入库 ── */
      '<div class="card mt16">' +
        '<div class="card-head">' + H.icon('upload', { size: 15 }) +
          '<span>一、影像序列入库</span>' +
          '<span class="card-tag">§5.12.3 · zip 解包 · 前端 JSZip</span>' +
        '</div>' +
        '<div class="card-body">' +
          ...
          '<div class="banner mb0 mt16">' + H.icon('info', { size: 15 }) +
            '<span>' + H.rich('**只收图片序列（JPG / PNG）**，zip 内按轴位 / 冠状 / 矢状分目录；前端 **JSZip 解包**，' +
              '**不依赖影像教学底座、不解析 DICOM**（§1.2 不做项）。' +
      ...
      '<div class="card mt16">' +
        '<div class="card-head">' + H.icon('shield', { size: 15 }) +
          '<span>二、脱敏信息</span>' +
          '<span class="card-tag">§5.12.4 · GEN-02 强制全掩</span>' +
        '</div>' +
        '<div class="card-body">' +
          '<div class="banner warn">' + H.icon('lock', { size: 15 }) +
            '<span>' + H.rich('`GEN-02`「患者基本情况完整」**强制全掩、不提供开关**：姓名等直接标识**在入库时即抹除**，' +
              '题库侧不存原文、也不存"可见 / 不可见"的选项。检查号 / 影像号**保留后 4 位**——' +
              '故该条属**部分不可评**，但**整体仍计入分母**（§5.2.2 甲类）。') + '</span>' +
          '</div>' +
          ...
          '</div>' +
          '<div class="field"><div class="field-label">临床简要（入学生可见的题干信息）<span class="req-star">*</span></div>' +
            ...
            '<div class="tiny muted" style="margin-top:5px">' +
              H.rich('此栏**学生可见**，只写与影像诊断相关的必要信息；**不得**出现姓名、住院号、身份证号等直接标识。') + '</div>' +
          '</div>' +
        '</div>' +
      '</div>' +
      ...
      '<div class="card mt16">' +
        '<div class="card-head">' + H.icon('sliders', { size: 15 }) +
          '<span>三、能力位声明</span>' +
          '<span class="card-tag">§5.12.5 · 控件派生位只读 × 样本声明位可勾</span>' +
        '</div>' +
        '<div class="card-body">' +
          ...
          '<table class="table table-compact">' +
            ...
            '<tbody>' +
              '<tr><td class="mono">hasMeasurement</td><td><span class="adm-cap locked">false 🔒</span></td>' +
                '<td>' + H.rich('**测量工具**本期归影像教学底座且未交付（§9.4 `measurement ❌`）→ **恒 false**。' +
          ...
          '<div class="adm-range ' + rangeCls + '" style="margin-top:16px;border-radius:10px;border:1px solid var(--border)">' +
            ...
            '<div class="tiny muted" style="margin-top:8px">' +
              H.rich('门禁线 85（§5.9.2）：**整卷加权**低于 85 时发布会走二次确认并写审计（§5.13.5）。' +
      ...
      '<div class="card mt16">' +
        '<div class="card-head">' + H.icon('file-pen', { size: 15 }) +
          '<span>四、金标准报告</span>' +
          '<span class="card-tag">§5.12.6 · 评分依据 · 学生端默认不可见</span>' +
        '</div>' +
        '<div class="card-body">' +
          (s.goldStandard ? '' :
            '<div class="banner error">' + H.icon('warn', { size: 15 }) +
              '<span>' + H.rich('**尚未录入金标准**：金标准是评分的唯一依据（§9.1），缺失时样本**只能是草稿、不可发布**。') + '</span></div>') +
          ...
          '<div class="banner mb0">' + H.icon('eye', { size: 15 }) +
            '<span>' + H.rich('金标准**在考核模式下对前端不可见**（§9.1 硬约束）；训练侧 T4 对照区需**提交自评后**才解锁，' +
              '且由任务级 `revealGoldStandardAfterSubmit` 控制是否下发原文（§5.8，默认 `false`）。') + '</span>' +
          '</div>' +
        '</div>' +
      '</div>' +

      '<div class="adm-sticky-foot" style="border-radius:12px;border:1px solid var(--border);margin-top:16px">' +
        '<span class="adm-foot-note">' + H.rich('保存即写 `updatedBy` / `updatedAt` 留痕；**已发布样本走新建版本**。') + '</span>' +
        '<div class="adm-foot-right">' +
          '<button class="btn sm" type="button" data-act="noop">' + H.icon('ban', { size: 12 }) + '停用样本</button>' +
  ...
  var CAP_WHY = {
    hasPriorExam: '样本带**既往片**。取 true 时 `IMP-08`「与既往片对照」可评；取 false 则该条落空（−4）。',
    hasEnhancedPhase: '样本含**增强序列**。取 true 时 `FIND-06`「强化程度描述」可评；取 false 则该条落空（−4）。',
    isTumor: '病变为**肿瘤性**。与 `hasStagingInfo` **合取**决定 `IMP-05`「分期依据」是否可评（附录 E）。',
