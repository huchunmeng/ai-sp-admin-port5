...
(function (w) {
  ...
  /* 考核任务状态 → 标签样式 · §5.5 状态全集（8 态，`已交卷` 是独立态，不等于 `评分中`） */
  var STATE_CLS = {
    '待作答': 'pending', '作答中': 'doing', '已交卷': 'submitted', '评分中': 'scoring',
  ...
  /* 模态：二次确认 / 申请复核 等。opt.validate = fn() → 返回错误文案则拦截不放行 */
  var modalOnClose = null, modalValidate = null;
  function openModal(opt) {
    var root = document.getElementById('modalRoot');
    if (!root) return;
    modalOnClose = opt.onClose || null;
    modalValidate = opt.validate || null;
    root.innerHTML =
  ...
  /* fire = true 时才执行回调（「取消 / 点遮罩」不该触发确认动作）；validate 不过则留在弹层内 */
  function closeModal(fire) {
    if (fire && modalValidate) {
      var err = modalValidate();
      if (err) { toast(err); return; }
    }
    var root = document.getElementById('modalRoot');
    if (root) root.innerHTML = '';
    var f = modalOnClose;
    modalOnClose = null; modalValidate = null;
    if (fire && f) f();
  }
  function dismissModal() { modalOnClose = null; modalValidate = null; closeModal(false); }

  /* 弹层内的字数计数（申诉原因等）：绑定 input 事件实时更新 #_cnt */
  function bindCounter(inputId, counterId, limit) {
    var el = document.getElementById(inputId), cnt = document.getElementById(counterId);
    if (!el || !cnt) return;
    var upd = function () { cnt.textContent = el.value.length + ' / ' + limit; };
    el.addEventListener('input', upd);
    upd();
  }
  function fieldErr(inputId, hintId, msg) {
    var el = document.getElementById(inputId), h = document.getElementById(hintId);
  ...
  w.H = {
    ...
    bindCounter: bindCounter, fieldErr: fieldErr,
    STATE_CLS: STATE_CLS
  };
})(window);
