...
**Font Awesome 图标本地化**（补记）— 五端 `index.html` 移除 cdnjs 外链，改为各端 `public/fontawesome/` 本地托管（`css/all.min.css` + 4 个 woff2：solid/regular/brands/v4compatibility），消除外网依赖，内网演示环境不再因 CDN 不可达而丢图标。共 30 个文件。
...
## 2026-09-18 — 产品更名「医路慧影（WiseImag）影像智思体」+ 训练端首页改造

### 已完成

**产品线更名** — 中大医院线产品名由「医道星途临床思维教学智能体」改为「**医路慧影（WiseImag）影像智思体**」，五端品牌名统一（此前只有训练端挂「医道星途」，其余四端是「AI-SP / AI标准化病人」）：

| 范围 | 改动 |
|------|------|
| 五端 `index.html` | `<title>` + `meta description` → 「医路慧影 · 管理端/训练端/考试端/电子书包/运营平台」 |
| 各端 Layout 品牌位 | 主位用全称「医路慧影（WiseImag）影像智思体」；窄位（运营平台 header-sub、sidebar-label、考试端侧栏）用简名「医路慧影」 |
| 零散品牌位 | 考试端登录页、管理端欢迎语、运营平台 `platformName`、App训练端版本行、两处 `document.title` |

定位规则：**品牌主位用全称、窄位用简名** —— 考试端侧栏 `.sys-name` 实测宽仅 167px，放 15 字全称会折成两行（高 40px vs 训练端单行 27px），故窄位一律简名，验证后回到单行 20px。

改名范围**仅限 port5 仓库 UI**；技术标识不动（`services/sp-api/`、`prompts/06-aisp/`、仓库名 `ai-sp-admin-port5`）；wiki 目录 `wiki/产品/医道星途/` 与工作台 OS 种子数据（44 处）仍为旧名，本次未处理。

**训练端首页改造** (`apps/training/src/views/HomeView.vue`)：

- **入口卡「SP训练」→「AI问诊」** —— 新名带「影像」，产品方向转向影像智思体，同时淡化「标准化病人(SP)」措辞（中间短暂试过「AI训练」，随即定稿「AI问诊」）
- **在线考试入口接外链** —— 卡片点击新标签页打开 `https://examon.mvwchina.com/`（`window.open` + `noopener,noreferrer`），角标改「外部系统」
- **新增放射诊断学 MOOC 横幅** —— 名医名课研习区块下方加独立一行深蓝渐变横幅（`elite-mooc`：图标 + 「放射诊断学MOOC / 中国大学MOOC 平台 · 名校名师系统课程」+「去学习」按钮），跳 `https://www.icourse163.org/`
- **「为你推荐」由病例卡改为科室入口卡** —— 原 3 条硬编码推荐病例卡，改为 10 张科室入口卡（彩色图标 + 「XX病例学习」+「N 例可用」+ 右箭头），5 列 2 行网格；**影像科置顶**呼应新品牌定位，其余依次内科/外科/妇产科/儿科/急诊科/精神科/皮肤科/骨科/神经内科
  - 例数由 mock 数据按 `specialtyGroup` 聚合，科室顺序与图标配色走 `SPECIALTY_ORDER` / `SPECIALTY_META` 常量，不额外维护科室列表
  - 中途曾实现「科室筛选 chip + 病例卡网格」方案，按反馈废弃 —— 需求是按科室**直接给多个入口**，不是先铺卡片再筛选
  - 顺带清理：`goSPTraining` → `goCaseList`（3 处调用点同步）、移除失效的 `spec-chip*` / `recommend-*` / `rec-*` 样式与 `getCaseLevel` 等 import。保留 `.diff-U/R/F`（学习记录的 `.record-diff` 仍在用）

playwright 实测：科室卡 10 张、顺序以影像科起头、点击跳 `/#/case-list`、console 零错误；首页其余区块（AI问诊/MDT/在线考试、名医名课研习 + MOOC、学习记录、系统通知）无回归。

### 待推进

1. **科室入口未按科室预过滤** —— 点任意科室入口目前统一跳 `caseList` 全量列表，不会只出该科室病例。要生效需给 `apps/training/src/views/CaseList.vue` 加 query 参数（如 `?specialty=影像科`）支持
2. **科室例数为 mock** —— 推荐池共 15 条（内科 4、影像科 2、外科 2、其余各 1），待接真实病例库后由后端聚合
3. **影像科病例缺真实数据源** —— 病例编辑器病种库 `apps/admin/src/views/case-editor/disease-data.js` 的 15 个专业里**没有放射科/影像科**（内科/儿科/急诊科/皮肤科/神经内科/精神科/普外/胸心外/泌尿外/整形外/骨科/儿外/妇产科/眼科/耳鼻喉科），影像科入口目前只靠 2 条 mock 撑着，扩充需先补病种分类
4. **全工作区改名（可选）** —— wiki 目录 `wiki/产品/医道星途/`、工作台 OS 种子数据、仓库根 `CLAUDE.md`（仍写「AI-SP 标准化病人平台」）均为旧名；若要做需连带动 `docs/MDT多学科讨论/DESIGN_02_模拟还原方案.md` 与 `项目日志.md` 里的路径引用
